/* Build options collection tool for pclp_config
   Copyright 2017 Gimpel Software
   All rights reserved
...
options.  The supported environment variables are:
  ...
  argument which appears to be a module.
...
#define MAX_PATH_SIZE 100*1000*1000
...
void log_yaml_arg(FILE *fd, const char *arg) {
    char c;
    fputc('\'', fd);
    if (modules_in_working_dir) {
        if (*arg && *arg != '-' && *arg != '.' && *arg != '/' && *arg != '\\' && *arg != '\'') {
            fputs(working_dir_buffer, fd);
            fputc('/', fd);
        }
    }
    if (rel_opt_intros) {
        ...
        while (*opt_intro) {
            ...
            if (!*argz || !*opt_intro || *opt_intro == ';') {
                if (*argz == '/' || (isalpha(*argz) && *(argz + 1) == ':')) {
                    break;
                }
                fwrite(arg, 1, argz - arg, fd);
                fputs(working_dir_buffer, fd);
                fputc('/', fd);
                arg = argz;
                break;
...
void log_args_yaml(FILE *fd, char * const argv[]) {
    ...
    while ((bytes_processed = get_next_delimited_string(vars_ptr, ";", env_var_buf, MAX_ENV_VAR_NAME, &string_size))) {
        ...
        while ((bytes_processed = get_next_delimited_string(include_ptr, include_delims, include_buffer, MAX_PATH_SIZE, &string_size))) {
            ...
            if (string_size > MAX_PATH_SIZE) {
				fprintf(stderr, "string_size out of range");
				continue;
			}
            if (argno++) { fputc(',', fd); }
            log_yaml_arg(fd, "-I");
