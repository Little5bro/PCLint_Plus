...
# Copyright Gimpel Software LLC 2017
# Confidential and proprietary, all rights reserved.
# No part of this file may be redistributed without express written permission
# of Gimpel Software LLC.
...
def processConfig(config_file):
    try:
        ...
        if not os.path.isabs(config_file):
            ...
            try:
                config = yaml.load(open(abs_config));
                return config
...
def generateCompilerConfig(config, args):
    ...
    if not compiler in config['compilers']:
        emit_error("'" + compiler + "' is not a valid compiler name\n")
    ...
    if args.config_output_lnt_file:
        with open(args.config_output_lnt_file, 'w') as f:
            ...
            if args.config_output_header_file:
                ...
                header_string = '+libh(' + args.config_output_header_file + ')\n' + '-header(' + args.config_output_header_file + ')\n'
...
def generateProjectConfig(config, args):
    ...
    compile_commands = yaml.load(imposter_contents)
